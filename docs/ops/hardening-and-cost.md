...
backup bucket, read on `/odoo/prod/*` parameters (plus `kms:Decrypt` scoped via
`kms:ViaService` to SSM), `sns:Publish` to `odoo-alerts`, and `AmazonSSMManagedInstanceCore`.

> **History:** started in Secrets Manager, moved to SSM Parameter Store on 2026-08-30 to cut
> the $0.40/month and keep one source of truth, moved back to Secrets Manager on 2026-09-02,
> then back to SSM again the same day — final call was to optimize for cost. Each swap was
> verified against live production (password authenticates, no container recreate needed)
> before the old store was deleted. Secrets Manager was force-deleted without a recovery
> window this time, since the intent was to stop the charge immediately, not park it.
